...
일반 로그는 호출이 있었다는 사실은 보여주지만, 관계는 사람이 직접 이어 읽어야 한다.
...
Blackbox를 사용하면 HTML export에서 source 호출과 target 쪽 처리가 같은 interaction id로 연결된다.
...
핵심 차이는 source -> target 연결이 각 객체의 활동 기록 옆에 유지되어, 실행 후에도 원인 흐름을 끊기지 않고 읽을 수 있다는 점이다.
