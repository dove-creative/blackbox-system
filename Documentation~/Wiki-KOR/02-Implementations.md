...
## 7-1. BLACKBOX 심볼이 없는 경우

`BlackboxHandle.Of(subject)`와 주요 기록 메서드는 `BLACKBOX` 심볼을 기준으로 동작한다. 이 심볼이 없으면 기록용 핸들이 유효하지 않거나 일부 기록 호출이 조건부로 빠질 수 있다. 따라서 Blackbox를 실제 추적 도구로 사용할 때는 빌드 설정에서 해당 심볼을 먼저 확인해야 한다.
