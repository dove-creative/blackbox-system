...
- `02-Plans.md`에 Unity Test Runner 축, 외부 NUnit `BLACKBOX` 활성 축, 외부 NUnit no-Blackbox 축을 구분해 적었다.
...
| `BLACKBOX` 활성 테스트 | `dotnet run --no-restore --project Tools/ExternalNUnitExecutor/ExternalNUnitExecutor.csproj` | 성공. `total=97 passed=97 failed=0` |
| `BLACKBOX` 비활성 테스트 | `dotnet run --no-restore --project Tools/ExternalNUnitExecutor/ExternalNUnitExecutor.NoBlackbox.csproj` | 성공. `total=1 passed=1 failed=0` |
