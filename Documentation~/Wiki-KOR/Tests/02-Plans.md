...
- 기존 `com.BlackThunder.BlackboxSystem.Tests.asmdef`는 유지한다.
- `com.BlackThunder.BlackboxSystem.Tests` assembly는 `com.BlackThunder.BlackboxSystem` assembly를 참조하고, `AssemblyInfo.cs`의 `InternalsVisibleTo("com.BlackThunder.BlackboxSystem.Tests")`를 통해 internal 타입을 검증한다.
- Unity Test Runner와 `com.BlackThunder.BlackboxSystem.Tests.asmdef` 축은 `UNITY_INCLUDE_TESTS`, `BLACKBOX`, `BLACKBOX_TESTS`가 켜진 상태를 기본으로 한다.
- 순수 NUnit 외부 실행 축은 `Tools/ExternalNUnitExecutor/ExternalNUnitExecutor.csproj`를 사용한다. 이 축도 `BLACKBOX`를 켠 상태에서 Blackbox의 실제 기록/출력 흐름을 검증한다.
- `BLACKBOX` 심볼이 꺼졌을 때의 fallback 동작은 `Tools/ExternalNUnitExecutor/ExternalNUnitExecutor.NoBlackbox.csproj` 같은 별도 no-Blackbox 축에서 확인한다. `com.BlackThunder.BlackboxSystem.Tests.asmdef`는 `BLACKBOX` define constraint를 가지므로 `SymbolOff` 행을 Unity asmdef 축에서 검증하려고 하지 않는다.
...
| `SymbolOffReturnsInvalidHandlesAndOriginalMessages` | `BLACKBOX` 심볼이 없는 축에서 public API가 invalid/default handle과 원래 message를 반환한다. |
