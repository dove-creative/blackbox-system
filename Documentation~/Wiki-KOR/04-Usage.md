...
## 1-1. BLACKBOX 심볼이 정의되어 있어야 한다

`BlackboxHandle`의 주요 기록 메서드는 `BLACKBOX` 심볼을 기준으로 동작한다. 이 심볼이 없으면 `BlackboxHandle.Of(subject)`는 유효한 핸들을 만들지 않고, 기록 호출은 로그를 남기지 않거나 원래 메시지를 그대로 반환한다.

즉 '코드는 넣었는데 로그가 안 남는다'면, 먼저 `BLACKBOX`가 정의되어 있는지부터 확인하는 편이 좋다.
