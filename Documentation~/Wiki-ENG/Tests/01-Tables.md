...
- **SymbolOff**: The build axis where the `BLACKBOX` symbol is missing. This axis is verified through a separate no-Blackbox verification path, not through `com.BlackThunder.BlackboxSystem.Tests.asmdef`.
...
This table has both the normal verification axis where the `BLACKBOX` symbol is on, and the fallback verification axis where the symbol is off. The `SymbolOff` row verifies, in a no-Blackbox build, that the public API returns original messages or default handles and does not create actual records.
...
|  | - |  |  |  |  |  |  |  |  |  |  |  |  |
| SymbolOff | - | **Default/Invalid** | X | - | - | - | - | `[return message]` | - | `[return message]` | `[return message]` | - | - |
