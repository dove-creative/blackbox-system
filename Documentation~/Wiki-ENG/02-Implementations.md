...
## 7-1. When the BLACKBOX Symbol Is Missing

`BlackboxHandle.Of(subject)` and the main recording methods operate based on the `BLACKBOX` symbol. Without this symbol, the recording handle may not be valid, or some recording calls may be conditionally skipped. Therefore, when using Blackbox as an actual tracing tool, first check that the symbol is configured in the build settings.
