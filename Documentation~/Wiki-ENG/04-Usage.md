...
## 1-1. The BLACKBOX Symbol Must Be Defined

The main recording methods of `BlackboxHandle` operate based on the `BLACKBOX` symbol. Without this symbol, `BlackboxHandle.Of(subject)` does not create a valid handle, and recording calls either leave no log or return the original message as is.

In short, if you inserted the code but no logs appear, first check whether `BLACKBOX` is defined.
